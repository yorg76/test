...
$('#barcodes_form').validate({
            ...
            invalidHandler: function (event, validator) { //display error alert on form submit   
                $('.alert-danger',$('#barcodes_form')).show();
                Metronic.scrollTo( $('.alert-danger',$('#add_user_form')), -200);
                $("#barcodes_form").removeAttr("novalidate");
	...
	$('#submit').click(function(e) {
		...
		if ($('#barcodes_form').validate().form()) {
            $('#barcodes_form').submit();
        }
...
/**
		...
		if (qz) {
			...
			try {
				console.log("qz ready...");		
				 $(window["qz"]).closest('.form-group').addClass('has-success');
				 $(window["qz"]).closest('.form-group').removeClass('has-error');
				 
				 $("textarea[name=qz_log]").append('Applet java załadowany poprawnie\n\n');
				 qz.findPrinter('E4');
				 window['qzDoneFinding'] = function qzDoneFinding() {
						$("textarea[name=qz_log]").append('Znaleziono drukarkę: '+qz.getPrinter()+'\n\n');
						
						$("#print_on_e4").attr('disabled',false);
						
						$("#print_on_e4").click(function(e) {
							
							e.preventDefault();
							
							var frame = $('[name=barcodes_frame]').contents();
							
							var barcodes = $('.barcode',frame);
							
							$.each(barcodes,function(index,value) {
								var barcode = $(this).val();	
								qz.append('SYSVAR(18)=0\n');
								qz.append('INPUT OFF\n');
								qz.append('SETUP "SERVICE,MEDIA SIZE,LENGTH,315"\n');
								qz.append('SETUP "SERVICE,MEDIA SIZE,XSTART,0"\n');
								qz.append('SETUP "SERVICE,MEDIA SIZE,WIDTH,832"\n');
								qz.append('SETUP "SERVICE,MEDIA TYPE,LABEL (w GAPS)"\n');
								qz.append('SETUP "DETECTION,FEEDADJ,STARTADJ,-152"\n');
								qz.append('SETUP "DETECTION,FEEDADJ,STOPADJ,0"\n');
								qz.append('SETUP "SERVICE,PERFORMANCE,NORMAL"\n');
								qz.append('SETUP "SERVICE,PRINT DEFS,NEW SUPPLIES,GZ-10"\n');
								qz.append('SETUP "SERVICE,PRINT DEFS,NEW SUPPLIES,GF100"\n');
								qz.append('SETUP "CONTRAST,5"\n');
								qz.append('SETUP "SERVICE,TESTFEED,LENGTH,315"\n');
								qz.append('LTS& OFF\n');
								qz.append('error 15,"Font not Found"\n');
								qz.append('error 18,"Disk Full"\n');
								qz.append('error 26,"Parameter to Large"\n');
								qz.append('error 27,"Parameter to Small"\n');
								qz.append('error 43,"Memory overflow"\n');
								qz.append('error 1003,"Field out of label"\n');
								qz.append('INPUT OFF\n');
								qz.append('FORMAT INPUT CHR$(2),CHR$(3),CHR$(13)\n');
								qz.append('KILL "LBLSOFT.LAY"\n');
								...
								qz.append('INPUT ON\n');
								qz.append('PF 3\n');
								qz.append('PRINT KEY OFF\n');
								
							})
							
							qz.print();
							
							 //qz.printToFile("E:\\print-debug.txt");
				...
				$(window["qz"]).closest('.form-group').addClass('has-error');
				$(window["qz"]).closest('.form-group').removeClass('has-success');
	...
	*/
	function qzDonePrinting() {
		// Alert error, if any
		if (qz.getException()) {
			$("textarea[name=qz_log]").append('Błąd drukowania:\n\n\t' + qz.getException().getLocalizedMessage()+"\n\n");
			qz.clearException();
			return; 
		}
		
		// Alert success message
		$("textarea[name=qz_log]").append('Pomyślnie wysłano kody na drukarkę "' + qz.getPrinter() + '" \n\n');
	}
	...
	function useDefaultPrinter() {
		if (isLoaded()) {
			...
			window['qzDoneFinding'] = function() {
				...
				$("textarea[name=qz_log]").append(printer !== null ? 'Domyślna drukarka znaleziona: "' + printer + '\n\n"':
					'Domyślna drukarka' + 'nie znaleziona\n\n');
	...
	function findPrinter(name) {
		...
		if (isLoaded()) {
			...
			window['qzDoneFinding'] = function() {
				...
				// Alert the printer name to user
				$("textarea[name=qz_log]").append(printer !== null ? 'Drukarka znaleziona: "' + printer + 
					'" szukano "' + p.value + '"' : ' Drukarka "' + 
					p.value + '" nie znaleziona.');
